...
def login(request):
    """
    登录功能，页面输入的数据与数据库作比较，如果匹配，进入index页面
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if db_password == password:
                responce = HttpResponseRedirect("/Buyer/index/")
                responce.set_cookie("username", user.username)
                responce.set_cookie("user_id", user.id)
...
def goods_list(request):
    ...
    keyword = request.GET.get('keywords')  # 查询的内容 t类型 k为 类型id  k类型 k为关键字
    goods_list = []  # 返回的结果
        ...
        if keyword:
            ...
            if keyword:
                goods_list = Goods.objects.filter(goods_name__contains=keyword).order_by(
                    '-goods_pro_time')  # 模糊查询查询商品名称含有关键字的商品
...
def pay_order(request):
    ...
    if goods_id and count:
        ...
        order.order_user = User.objects.get(id=int(request.COOKIES.get("user_id")))  # 订单对应的买家
        order.save()
        ...
        order_info.store_id = goods.goods_store  # 商家卖家，goods.goods_store 本身就是一条卖家数据
        order_info.save()
...
def add_cart(request):
    ...
    if request.method == "POST":
        ...
        goods = Goods.objects.get(id=id)  # 获取点商品信息
        cart = Cart()
...
def cart(request):
    ...
    count = goods.count()
    # goods_all = Cart.objects.all()
    # total=goods_all.goods_total()
...
def middle_test_view(request):
    ...
    return JsonResponse({"data":"hello handsome boy"})

from django.core.cache import cache
def cacjeTest(request):
    user=cache.get("user") #从缓存里面获取用户
    if not user:
        user=User.objects.get(id=1)
        cache.set("user",user,30) #将用户数据存入缓存，缓存时间30s
