...
def goods_add(request):
    ...
    if request.method == 'POST':
        ...
        goods.goods_pro_time = data.get("goods_pro_time")  # 出厂日期格式必须是yyyy-yy-yy 格式
        goods.goods_status = 1
...
def send_login_code(request):
    ...
    if request.method == "POST":
        ...
        c.code_content = code
        c.save()
        send_data = "%s的验证码是%s，千万不要告诉别人" % (email, code)
        sendDing(send_data)
        result['data'] = "发送成功"
    else:
        result["code"] = 400
...
def order_list(request,status):
   ...
   """
    ...
    store=User.objects.get(id=user_id) #获取店铺信息
